...
from pyspark.sql.functions import col, when, lit, udf, regexp_replace, substring, length, round, format_number, concat
...
def number_to_strings_spark_fixed(
    data: DataFrame,
    column_name: str,
    prefix: str = "",
    suffix: str = "",
    ...
    """
    Convert a Spark DataFrame column of numbers to strings, handling prefix and suffix with proper formatting.
    """
    # First round the values to avoid floating point precision issues
    result = data.withColumn(column_name, round(col(column_name), rounding))
    
    # Convert to string without automatic formatting (no thousands separators)
    result = result.withColumn(column_name, col(column_name).cast(StringType()))
    
    if prefix:
        result = result.withColumn(column_name, concat(lit(prefix), col(column_name)))
    if suffix:
        result = result.withColumn(column_name, concat(col(column_name), lit(suffix)))
    return result


class SparkConverterFixed:
    def __init__(
        self,
    ...
    def fit(self, data: DataFrame) -> "SparkConverterFixed":
    ...
    def inverse_convert(self, data: DataFrame) -> DataFrame:
        ...
        if self._type == str:
            data = number_to_strings_spark_fixed(data, self._column_name, self._prefix, self._suffix, self._rounding)
